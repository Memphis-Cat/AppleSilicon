...
`p1.07-trace-events` is the version-controlled initial MMIO trace set. Runtime harnesses verify every configured event against `qemu-system-aarch64 -trace help` before launch.

`p1.08-compare.json` defines deterministic trace-analysis behavior, including selected event names, context size, bounded resynchronization, guest fields that must be preserved, and host-only fields that may be normalized away.

`p1.09-manifest-policy.json` defines the versionable evidence contract. It pins the research source/revision, defines reference/probe CPU and accelerator roles, specifies which fields must match across an A/B pair, limits artifact kinds, and declares privacy-sensitive keys/command markers.

`p1.09-reference.example.json` and `p1.09-probe.example.json` are sanitized examples with synthetic digest values. They are designed to satisfy the P1.09 pairing contract while differing only in the expected host, accelerator, and CPU dimensions.

Configuration files may describe placeholders, but must not contain:

- Apple proprietary firmware,
- macOS disk images,
- raw VM machine identifiers or UUIDs,
