...
extract_last() {
    local label="$1"
    local path="$2"
    sed -n "s/^${label}: //p" "${path}" | tail -n 1
}

extract_first() {
    local label="$1"
    local path="$2"
    sed -n "s/^${label}: //p" "${path}" | head -n 1
}
...
require_file "INPUT_MACHINE_IDENTITY_MISSING" "VMApple machine identity" "${MACHINE_IDENTITY}"
if [[ -n "${HARDWARE_MODEL}" ]]; then
    require_file "INPUT_HARDWARE_MODEL_MISSING" "VMApple hardware model" "${HARDWARE_MODEL}"
fi
...
for pair in \
    ...
    "QEMU debug log=${QEMU_DEBUG_LOG}" \
    "Trace capability log=${TRACE_HELP_LOG}"; do
    key="${pair%%=*}"
    value="${pair#*=}"
    [[ -n "${value}" ]] || fail "PROBE_METADATA_INCOMPLETE" "Missing ${key} in P1.07 launcher log."
done

[[ "${RESULT}" == P1_07_PROBE_* ]] || fail "PROBE_RESULT_NOT_RUNTIME" "Launcher result is not a completed P1.07 runtime classification: ${RESULT}"
[[ "${ACCEL}" == "tcg" ]] || fail "PROBE_ACCELERATOR_INVALID" "Probe launcher must report TCG, observed: ${ACCEL}"
case "${CPU_PROFILE}" in
    max|apple-gxf) ;;
    *) fail "PROBE_CPU_INVALID" "Probe CPU must be max or apple-gxf, observed: ${CPU_PROFILE}" ;;
esac
[[ "${SMP}" =~ ^[0-9]+$ ]] && (( SMP > 0 )) || fail "PROBE_SMP_INVALID" "Invalid SMP value: ${SMP}"
...
CURRENT_OS="$(uname -s 2>/dev/null || printf '%s' unknown)"
CURRENT_ARCH="$(uname -m 2>/dev/null || printf '%s' unknown)"
[[ "${CURRENT_OS}" == "${HOST_OS}" && "${CURRENT_ARCH}" == "${HOST_ARCH}" ]] \
    || fail "PROBE_HOST_CHANGED" "Collector must run on the same host immediately after the probe. Launcher=${HOST_OS}/${HOST_ARCH}, current=${CURRENT_OS}/${CURRENT_ARCH}."

if [[ "${RAM}" == *G ]]; then
    RAM_VALUE="${RAM%G}"
    [[ "${RAM_VALUE}" =~ ^[0-9]+$ ]] || fail "PROBE_RAM_INVALID" "Unsupported RAM value: ${RAM}"
    RAM_MIB=$((RAM_VALUE * 1024))
elif [[ "${RAM}" == *M ]]; then
    RAM_MIB="${RAM%M}"
    [[ "${RAM_MIB}" =~ ^[0-9]+$ ]] || fail "PROBE_RAM_INVALID" "Unsupported RAM value: ${RAM}"
else
    fail "PROBE_RAM_INVALID" "RAM must use G or M units, observed: ${RAM}"
fi

if command -v sysctl >/dev/null 2>&1; then
    HOST_CPU_FAMILY="$(sysctl -n machdep.cpu.brand_string 2>/dev/null || true)"
else
    HOST_CPU_FAMILY=""
fi
if [[ -z "${HOST_CPU_FAMILY}" ]] && command -v lscpu >/dev/null 2>&1; then
    HOST_CPU_FAMILY="$(lscpu 2>/dev/null | sed -n 's/^Model name:[[:space:]]*//p' | head -n 1)"
fi
...
MANIFEST_ARGS=(
    ...
    --host-virtualization TCG
    --accelerator tcg
    --cpu-model "${CPU_PROFILE}"
    --ram-mib "${RAM_MIB}"
    --smp "${SMP}"
    ...
    --artifact "launcher_log=${LAUNCHER_LOG}"
...
while IFS= read -r event || [[ -n "${event}" ]]; do
    ...
    MANIFEST_ARGS+=(--trace-event "${event}")
done < "${TRACE_CONFIG}"

IFS=',' read -r -a DEBUG_ARRAY <<< "${DEBUG_ITEMS}"
for item in "${DEBUG_ARRAY[@]}"; do
    [[ -n "${item}" ]] && MANIFEST_ARGS+=(--debug-item "${item}")
done
...
python3 "${MANIFEST_TOOL}" validate "${OUTPUT_MANIFEST}" --policy "${MANIFEST_POLICY}" \
    || fail "PROBE_MANIFEST_INVALID" "Generated probe manifest failed P1.09 validation."
...
echo "This command does not launch QEMU; it packages an already completed P1.07 runtime result into the P1.09 evidence contract."
