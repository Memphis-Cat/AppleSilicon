...
PY
)" || fail "P3_05_UNSAFE_WORK_ROOT" "Could not canonicalize P3.05 work root: ${WORK_ROOT}"
    normalized_build_root="$(python3 - "${ROOT_DIR}/.build" <<'PY'
from pathlib import Path
import sys
print(Path(sys.argv[1]).resolve(strict=False))
PY
)" || fail "P3_05_UNSAFE_WORK_ROOT" "Could not canonicalize project build root"
    ...
    [[ "${normalized_root}" == "${normalized_build_root}/"* ]] ||
        fail "P3_05_UNSAFE_WORK_ROOT" "Work root must remain below ${normalized_build_root}"
